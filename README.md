...
Tento RPi ma pripraveny `CLAUDE.md` — kontextovy soubor pro Claude Code.
Kdyz ho nahrajes, Claude automaticky vi jak je system nakonfigurovan.
...
> Pozn.: npm neni na RPi predinstalovan. Nainstaluj pres nvm:
> ```bash
...
### Stazeni CLAUDE.md z privátního repozitare

```bash
TOKEN="ghp_xxxxxxxxxxxxxxxxxxxx"
curl -fsSL \
  -H "Authorization: token $TOKEN" \
  "https://raw.githubusercontent.com/romankysely/MeshCore/moje-zmeny/mcbridge-setup/CLAUDE.md" \
  -o ~/CLAUDE.md
```

Soubor `CLAUDE.md` je ulozeny v privatnim repozitari `romankysely/MeshCore`, vetev `moje-zmeny`, slozka `mcbridge-setup/`.
